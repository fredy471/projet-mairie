...
if (!isset($_SESSION['id']) && !isset($_SESSION['role'])) {
    ...
    $req = 'SELECT * FROM demandes d JOIN users u ON d.id_citoyen=u.id_user WHERE id_agent=:id_user ORDER BY date DESC';
    $stt = $conn->prepare($req);
    $stt->bindParam(':id_user', $id_user, PDO::PARAM_INT);
    $stt->execute();
    $demandes = $stt->fetchAll(PDO::FETCH_ASSOC);

    //Nbre de demandes en fonction du type
    $req_req = 'SELECT COUNT(id_demande) as nbre, types FROM demandes WHERE statut="en attente" GROUP BY types ORDER BY nbre DESC ';
    $stt_stats = $conn->prepare($req_req);
    $stt_stats->execute();
    $stats = $stt_stats->fetchAll(PDO::FETCH_ASSOC);

    //total des demandes
    $req2 = "SELECT * FROM demandes WHERE statut = 'en attente'";
    $declar = $conn->prepare($req2);
    $declar->execute();


    // total des demandes TRAITÉES (par l'agent ACTUEL)
    $req_one = "SELECT COUNT(*) as nbretotal FROM demandes WHERE id_agent = :id_user";
    $traites = $conn->prepare($req_one);
    $traites->bindParam(':id_user', $id_user, PDO::PARAM_INT);
    $traites->execute();
