...
if (!isset($_SESSION['id']) && !isset($_SESSION['role'])) {
    ...
    //total des demandes recemment traitees par l'agent
    $req = 'SELECT * FROM demandes d JOIN users u ON d.id_citoyen=u.id_user WHERE id_agent is not null ORDER BY date DESC LIMIT 4';
    $stt = $conn->prepare($req);
    $stt->execute();
    $demandes = $stt->fetchAll(PDO::FETCH_ASSOC);

    //Nbre de demandes en fonction du type
    $req_req = 'SELECT COUNT(id_demande) as nbre, types FROM demandes GROUP BY types ORDER BY nbre DESC';
    $stt_stats = $conn->prepare($req_req);
    $stt_stats->execute();
    $stats = $stt_stats->fetchAll(PDO::FETCH_ASSOC);

    //total des demandes
    $req2 = "SELECT * FROM demandes";
    $declar = $conn->prepare($req2);
    $declar->execute();


    // total des demandes TRAITÉES
    $req_one = "SELECT COUNT(*) as nbretotal FROM demandes WHERE id_agent is not null";
    $traites = $conn->prepare($req_one);
    $traites->execute();
    $result = $traites->fetch(PDO::FETCH_ASSOC);

    //nomrbe de demandes traitees par agent

    $request = 'SELECT u.nom, u.prenom, COUNT(d.id_demande) AS nbre
    FROM users u
    JOIN demandes d ON u.id_user = d.id_agent
    GROUP BY u.id_user
    ORDER BY nbre DESC';
    $stat_agent = $conn->prepare($request);
    $stat_agent->execute();
    $stats_for_agent = $stat_agent->fetchAll(PDO::FETCH_ASSOC);
        ...
        <div class="main-content">
            ...
            <main class="">
                    ...
                    <div class="row">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="server-load row">
                                        <div class="server-stat col-sm-8 offset-sm-2">
                                            <p class='text-primary'><?= $declar->rowCount() ?></p>
                                            <span>Total demandes</span>
                                        </div>
                                        <div class="server-stat col-sm-8 offset-sm-2">
                                            <p class='text-success'><?= $result['nbretotal'] ?></p>
                        ...
                        <div class="col-lg-8">
                            <div class="card card-transactions">
                                <div class="card-body">
                                    <h5 class="card-title"><i class="fa-solid fa-chart-column text-success"></i> Statistiques par type de demande</h5>
                                    <div class="table-responsive">
                                        <table class="table table-striped">
                                            <thead>
                                                <tr>
                                                    <th>Types de demandes</th>
                                                    <th>Nombre</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php foreach ($stats as $stat) { ?>
                                                    <tr>
                                                        <td><?= htmlspecialchars($stat['types']) ?></td>
                                                        <td><?= $stat['nbre'] ?></td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
                ...
                <div class="container">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">📋 traitements recents </h5>
                            <div class="table-responsive">
                ...
                <div class="container" style='margin-top:50px;'>
                    <div class="row">
                        <div class="col-md-6 mx-auto justify-content-center">
                            <h5 class="">Statistiques des agents traitants</h5>
                            <table class='table table-hover align-middle table-rsponsive'>
                                <thead class='table-primary'>
                                    <tr>
                                ...
                                <tbody>
                                    <?php foreach ($stats_for_agent as $stats) { ?>
                                        <tr>
                                            <td><?= htmlspecialchars($stats['nom'] . ' ' . $stats['prenom']) ?></td>
                                            <td><?= htmlspecialchars($stats['nbre']) ?></td>
                                        </tr>
                                    <?php } ?>
